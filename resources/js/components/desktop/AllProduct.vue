    ...
    <div>
        <div class="row col2-flex">
            ...
            <div class="col-80p">
                <div v-if="!is_loading">
                    <div class="row-100">
                        <h5 class="fp-black" style="padding: 0 0 3px 5px;">Hasil Pencarian : {{keyword}} ({{total_record}} produk)</h5>
                    </div>
                    <div class="row-100">
...
<script>
    ...
    import Category from "./component/filter/Category.vue";
    ...
    export default {
        ...
        methods: {
            close_filter(){
                ...
                $('.slider-filter').removeClass('overflow-scroll');
            ...
            doFilter(event){
                ...
                $(".btn-close").click();
                apiCustomer.filterProduct({filter: this.filter}).then(response => {
            ...
            filterPagination(filter){
                apiCustomer.filterProduct({page: this.page, filter: filter}).then(response => {
                    if(response.data.code === 200){
                        this.is_filter = true;
                        //this.PList = response.data.data.product;
                        this.is_searching = false;
            ...
            Pagination(page){
                this.$router.replace({path:'/product', query : {page : page , search : this.keyword}})
                this.page =  page
                this.is_searching = true
                if(this.is_filter){
                    this.filterPagination(this.filter)
                } else {
                    this.search();
                }
            },
            ...
            search(){
                apiCustomer.searchProduct({keyword : this.keyword, page : this.page}).then(response => {
                    this.PList = response.data.data.product;
                    this.is_loading = false;
                    this.is_searching = false;
                    this.total_page = response.data.data.total_page
