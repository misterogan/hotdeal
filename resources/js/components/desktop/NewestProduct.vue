<template>
    <div class="col">
        <div id="ob_product_newest" class="mbottom-30" v-if="is_loading">
        ...
        <div v-if="!is_loading && Object.keys(latest).length > 0">
            <div class="row align-center mbottom-15">
                <h3 class="title-section">Produk Terbaru</h3>
                <router-link class="see-all" to="/product?search=">Lihat Semua</router-link>
            </div> 
            <div class="section-product wishlist">
                <small-card-product-without-stock-highlight :item="item" v-for="(item , index) in latest"  :key="index" @updateCarts="$emit('updateCarts' , $event)"></small-card-product-without-stock-highlight>
            </div>
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        name: "NewestProduct.vue",
        mounted(){
            this.getLatestProduct()
        },
        methods: {
            getLatestProduct(){
                const element = document.querySelector("#ob_product_newest");
                this.observer = new IntersectionObserver(entries => {
                    const target = entries[0];
                    if (target.isIntersecting) {
                        apiCustomer.highlight_new_product().then( response => {
                            this.latest = response.data.data.products;
